...
class Taken {
    ...
    writeSeeData(data) {
        ...
        if (this.state['myTeam'].length !=0 && this.state.pos){
            let minDist = Infinity;
            this.state?.myTeam.forEach(teammate => {
                console.log("CALC NEAREST TEAMMATE", teammate, this.state.pos)
                let currentDist = calculations.distance(teammate, this.state.pos)
                if (currentDist < minDist) {
                    this.last_seen_teammate = teammate
                    minDist = currentDist
                }
            });
        }
